...
void Reaction::SetProton(int i) {
  ...
  // if (abs(((_prot_mom_tmt) / (_prot_mom_uncorr)) - 1) > 0.05)
  //   std::cout << " prot fd ?  " << _is_FD_Prot << "  eloss prot  " << ((_prot_mom_tmt) / (_prot_mom_uncorr)) << std::endl;
  ...
  // if(abs(fpro-1) > 0.05 )
  //  std::cout<<" prot fd ?  "<<_is_FD_Prot<<   "  fpro  "<<fpro<<std::endl;
...
void Reaction::SetPip(int i) {
  ...
  // if (abs(fpip - 1) > 0.05) std::cout << " pip fd ?  " << _is_FD_Pip << "  fpip  " << fpip << std::endl;
...
void Reaction::SetPim(int i) {
  ...
  // if (abs(fpim - 1) > 0.05) std::cout << " pim fd ?  " << _is_FD_Pim << "  fpim " << fpim << std::endl;
...
// // //// Now Our version of Momentum corrections: TWOPION all w range:
...
// // // // // //// Now Our version of Momentum corrections, w< 2.55
...
float_t Reaction::scalar_triple_product_lab() {
  _rot_prot_Vect3 = _prot->Vect();
  _rot_pip_Vect3 = _pip->Vect();
  _rot_pim_Vect3 = _pim->Vect();
  if (TwoPion_exclusive()) {
    return (_lab_prot_Vect3.Dot(_lab_pip_Vect3.Cross(_lab_pim_Vect3)));
...
void Reaction::boost() {
  ...
  _rot_prot_Vect3 = _rotated_prot->Vect();
  _rot_pip_Vect3 = _rotated_pip->Vect();
  _rot_pim_Vect3 = _rotated_pim_measured->Vect();
...
float_t Reaction::scalar_triple_product_rot() {
  if (!_is_boosted) boost();
  if (TwoPion_exclusive()) {
    return (_rot_prot_Vect3.Dot(_rot_pip_Vect3.Cross(_rot_pim_Vect3)));
