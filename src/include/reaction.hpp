...
class Reaction {
 ...
 public:
  ...
  void SmearingFunc(int part_id, int status_part, double p, double theta, double phi, double &pNew, double &thetaNew,
    ...
    if (part_id == ELECTRON) {
      phiNew = phi + phiR * gRandom->Gaus(0, 1) * 0.4 *
                         syst_multi_fact;  // * ((-0.4632) * w_val + (2.0038) * w_val + (-0.9035) * w_val); /// 0.4 was
                                           // the origonal from pass1
      thetaNew = theta + thetaR * gRandom->Gaus(0, 1) * 0.4 *
                             syst_multi_fact;  // * ((-0.4632) * w_val + (2.0038) * w_val + (-0.9035) * w_val);
  ...
  inline float betaPip() { return _beta_pip; }
  ...
  // check the fraction, check with mc (similarities), events from target window(small contributions)
