...
double dt_cut_fd_up[3][6];
double dt_cut_fd_down[3][6];
double dt_cut_cd_up[3][3];
double dt_cut_cd_down[3][3];

if (!_mc) {
  // Experimental data dt cuts
  double exp_dt_cut_fd_up[3][6] = {{-0.0055, 0.10236, -0.7266, 2.447, -3.926, 2.893},
                                   {-0.003983, 0.0719, -0.4958, 1.634, -2.598, 2.059},
                                   {-0.002695, 0.0391, -0.2146, 0.56, -0.7153, 0.743}};
  double exp_dt_cut_fd_down[3][6] = {{0.005783, -0.1055, 0.7295, -2.377, 3.672, -2.697},
                                     {0.005386, -0.0987, 0.6816, -2.203, 3.326, -2.277},
                                     {0.00419, -0.06027, 0.3281, -0.8403, 1.025, -0.8545}};
  double exp_dt_cut_cd_up[3][3] = {{0.06, -0.3303, 0.7656}, {0.01909, 0.00434, 0.428}, {-0.005173, 0.09735, 0.3801}};
  double exp_dt_cut_cd_down[3][3] = {
      {-0.04974, 0.286, -0.73}, {-0.03662, 0.1555, -0.4675}, {-0.02998, 0.1083, -0.4429}};

  // Copy experimental cuts to dt_cut arrays
  memcpy(dt_cut_fd_up, exp_dt_cut_fd_up, sizeof(exp_dt_cut_fd_up));
  memcpy(dt_cut_fd_down, exp_dt_cut_fd_down, sizeof(exp_dt_cut_fd_down));
  memcpy(dt_cut_cd_up, exp_dt_cut_cd_up, sizeof(exp_dt_cut_cd_up));
  memcpy(dt_cut_cd_down, exp_dt_cut_cd_down, sizeof(exp_dt_cut_cd_down));

} else {
  // Simulation data dt cuts
  double sim_dt_cut_fd_up[3][6] = {{-0.003056, 0.05838, -0.429, 1.513, -2.592, 2.297},
                                   {-0.0013075, 0.02531, -0.1893, 0.6865, -1.227, 1.377},
                                   {-0.0009727, 0.0131, -0.0642, 0.1498, -0.2065, 0.6343}};
  double sim_dt_cut_fd_down[3][6] = {{0.004017, -0.07574, 0.5454, -1.866, 3.047, -2.426},
                                     {0.002022, -0.0358, 0.2452, -0.823, 1.369, -1.379},
                                     {0.002928, -0.04208, 0.2229, -0.5396, 0.6313, -0.7734}};
  double sim_dt_cut_cd_up[3][3] = {{0.05585, -0.2876, 0.714}, {0.0458, -0.1715, 0.5796}, {0.014305, -0.04828, 0.5063}};
  double sim_dt_cut_cd_down[3][3] = {
      {-0.02893, 0.1836, -0.649}, {-0.0731, 0.303, -0.58745}, {-0.03882, 0.1859, -0.519}};

  // Copy simulation cuts to dt_cut arrays
  memcpy(dt_cut_fd_up, sim_dt_cut_fd_up, sizeof(sim_dt_cut_fd_up));
  memcpy(dt_cut_fd_down, sim_dt_cut_fd_down, sizeof(sim_dt_cut_fd_down));
  memcpy(dt_cut_cd_up, sim_dt_cut_cd_up, sizeof(sim_dt_cut_cd_up));
  memcpy(dt_cut_cd_down, sim_dt_cut_cd_down, sizeof(sim_dt_cut_cd_down));
}
...
bool Pass2_Cuts::EC_sampling_fraction_cut() {
  ...
  double mean_minus_3_5_sigma[6][3];
  double mean_plus_3_5_sigma[6][3];

  if (!_mc) {
    // Assign values for the non-MC case
    double non_mc_minus[6][3] = {{-0.0001186, 0.0001892, 0.1942}, {-0.000856, 0.01084, 0.1637},
                                 {-0.001184, 0.014046, 0.1593},   {-0.001268, 0.01918, 0.1287},
                                 {-0.0002744, 0.003532, 0.1844},  {-0.001039, 0.012505, 0.1593}};
    double non_mc_plus[6][3] = {{-0.0004027, 0.001746, 0.2903}, {-9.36e-05, -0.000999, 0.2979},
    ...
    for (int i = 0; i < 6; ++i) {
      for (int j = 0; j < 3; ++j) {
        mean_minus_3_5_sigma[i][j] = non_mc_minus[i][j];
        mean_plus_3_5_sigma[i][j] = non_mc_plus[i][j];
      }
    }
  } else {
    // Assign values for the MC case
    double mc_minus[6][3] = {{-0.00058, 0.00687, 0.19312}, {-0.00088, 0.01022, 0.18360}, {-0.00089, 0.00941, 0.18832},
                             {-0.00066, 0.00888, 0.18466}, {-0.00066, 0.00798, 0.18884}, {-0.00055, 0.00685, 0.19319}};
    double mc_plus[6][3] = {{-0.00002, -0.00078, 0.29991}, {0.00023, -0.00396, 0.31026}, {0.00010, -0.00156, 0.30077},
                            {0.00017, -0.00400, 0.31052},  {0.00018, -0.00342, 0.30823}, {0.00012, -0.00297, 0.30706}};
    // Copy values directly
    for (int i = 0; i < 6; ++i) {
      for (int j = 0; j < 3; ++j) {
        mean_minus_3_5_sigma[i][j] = mc_minus[i][j];
        mean_plus_3_5_sigma[i][j] = mc_plus[i][j];
      }
    }
  }
