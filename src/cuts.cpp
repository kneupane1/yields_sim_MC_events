...
bool Cuts::ElectronCuts() {
        ...
        _elec &= (_data->vz(0) > -(2.78 + 2 * 2.16) &&
                  _data->vz(0) < (-2.78 + 2 * 2.16)); // 3 sigma cut
...
bool uconn_Cuts::EC_sampling_fraction_cut() {
        ...
        double sigma = ecal_e_sampl_sigm[0][isec] + ecal_e_sampl_sigm[1][isec] / (10 * (_data->p(0) - ecal_e_sampl_sigm[2][isec]));
        double upper_lim_total = mean + sigma_range * sigma;
        double lower_lim_total = mean - sigma_range * sigma;
        bool pass_band = _data->ec_tot_energy(0) / _data->p(0) <= upper_lim_total && _data->ec_tot_energy(0) / _data->p(0) >= lower_lim_total;
...
//////////////////////////////////////////////////////////////
        ...
        return (_data->ec_pcal_lv(0) > min_v && _data->ec_pcal_lv(0) < max_v && _data->ec_pcal_lw(0) > min_w && _data->ec_pcal_lw(0) < max_w);
...
bool uconn_Cuts::DC_fiducial_cut_XY() {
        ...
        return ((Y1 > calc_min1) && (Y1 < calc_max1) && (Y2 > calc_min2) && (Y2 < calc_max2) && (Y3 > calc_min3) && (Y3 < calc_max3));
        // _dc_fid_cut &= (Y1 < calc_max1);
        // return _dc_fid_cut;
...
//
        ...
        int region3 =3;
        // std::cout << "pid = " <<pid<< '\n';
        double calc_phi_min1 = minparams_in[pid][dc_sector-1][region1-1][0] + minparams_in[pid][dc_sector-1][region1-1][1] * log(theta_DCr1) + minparams_in[pid][dc_sector-1][region1-1][2] * theta_DCr1 + minparams_in[pid][dc_sector-1][region1-1][3] * theta_DCr1 * theta_DCr1;

        double calc_phi_max1 = maxparams_in[pid][dc_sector-1][region1-1][0] + maxparams_in[pid][dc_sector-1][region1-1][1] * log(theta_DCr1) + maxparams_in[pid][dc_sector-1][region1-1][2] * theta_DCr1 + maxparams_in[pid][dc_sector-1][region1-1][3] * theta_DCr1 * theta_DCr1;
        ...
        //
        double calc_phi_min2 = minparams_in[pid][dc_sector-1][region2-1][0] + minparams_in[pid][dc_sector-1][region2-1][1] * log(theta_DCr2) + minparams_in[pid][dc_sector-1][region2-1][2] * theta_DCr2 + minparams_in[pid][dc_sector-1][region2-1][3] * theta_DCr2 * theta_DCr2;

        double calc_phi_max2 = maxparams_in[pid][dc_sector-1][region2-1][0] + maxparams_in[pid][dc_sector-1][region2-1][1] * log(theta_DCr2) + maxparams_in[pid][dc_sector-1][region2-1][2] * theta_DCr2 + maxparams_in[pid][dc_sector-1][region2-1][3] * theta_DCr2 * theta_DCr2;

        // return (phi_DCr2 > calc_phi_min2) && (phi_DCr2 < calc_phi_max2);

        double calc_phi_min3 = minparams_in[pid][dc_sector-1][region3-1][0] + minparams_in[pid][dc_sector-1][region3-1][1] * log(theta_DCr3) + minparams_in[pid][dc_sector-1][region3-1][2] * theta_DCr3 + minparams_in[pid][dc_sector-1][region3-1][3] * theta_DCr3 * theta_DCr3;

        double calc_phi_max3 = maxparams_in[pid][dc_sector-1][region3-1][0] + maxparams_in[pid][dc_sector-1][region3-1][1] * log(theta_DCr3) + maxparams_in[pid][dc_sector-1][region3-1][2] * theta_DCr3 + maxparams_in[pid][dc_sector-1][region3-1][3] * theta_DCr3 * theta_DCr3;

        return ((phi_DCr1 > calc_phi_min1) && (phi_DCr1 < calc_phi_max1) && (phi_DCr2 > calc_phi_min2) && (phi_DCr2 < calc_phi_max2) && (phi_DCr3 > calc_phi_min3) && (phi_DCr3 < calc_phi_max3));
}
...
bool uconn_Cuts::Hadron_Chi2pid_cut(int i) {
        ...
        if(isstrict) {
                ...
                else if(p<4.6) chi2cut = chi2pid < coef*(0.00869 + 14.98587*exp(-p/1.18236) + 1.81751*exp(-p/4.86394));
                else chi2cut = chi2pid<coef*(-1.14099 + 24.14992*exp(-p/1.36554) + 2.66876*exp(-p/6.80522));
        } else {
                if(p<2.44) chi2cut = chi2pid < 3*coef;
                else chi2cut = chi2pid < coef*(0.00869 + 14.98587*exp(-p/1.18236) + 1.81751*exp(-p/4.86394));
        }
